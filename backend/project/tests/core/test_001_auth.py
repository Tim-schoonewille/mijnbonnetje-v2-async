...
from app.utilities.core.token import (
...
def test_auth_signup(client):
    ...
    response = client.get(
        '/auth/email/verify/',
        params={'token': email_verification_token}
    )
...
def test_auth_signin_invalid_email(client):
    response = client.post(
        '/auth/signup',
        json={'email': 'no-email-addres.com', 'password': 'password'}
    )
...
def test_auth_login_failure(client):
    response = client.post(
        'auth/login',
        json={'email': 'asdfsaf@gaymail.com', 'password': 'asdfasdfsaf'}
    )
    data = response.json()
