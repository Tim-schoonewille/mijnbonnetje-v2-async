...
def test_revoke_refresh_tokens(client):
    new_login = client.post(
        '/auth/login',
        json={
            'email': 'tim.schoonewille@gmail.com',
...
def test_sudo_read_user_wrong_parameter(client):
    ...
    expected_output = {
        'detail': [
                ...
                {
                    'type': 'is_instance_of',
                    'loc': ['path', 'user_id', 'lax-or-strict[lax=json-or-python[json=function-after[uuid_validator(), union[str,bytes]],python=union[is-instance[UUID],function-after[uuid_validator(), union[str,bytes]]]],strict=json-or-python[json=function-after[uuid_validator(), union[str,bytes]],python=is-instance[UUID]]]', # noqa
                    'is-instance[UUID]'], # noqa
                ...
                },
                {
                    'type': 'uuid_parsing',
                    'loc': ['path', 'user_id', 'lax-or-strict[lax=json-or-python[json=function-after[uuid_validator(), union[str,bytes]],python=union[is-instance[UUID],function-after[uuid_validator(), union[str,bytes]]]],strict=json-or-python[json=function-after[uuid_validator(), union[str,bytes]],python=is-instance[UUID]]]', # noqa
                    'function-after[uuid_validator(), union[str,bytes]]'], # noqa
...
async def test_api_call_log_limit(client):
    ...
    for i in range(100):
        response = client.get(
            f'{settings.URL_PREFIX}/test/api-call-log-limit'
        )
        if i == 3:
            assert response.status_code == 200
        if i == 11:
