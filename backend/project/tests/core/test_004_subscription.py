...
def test_create_and_get_subscription(client):
    ...
    new_subscription = client.post(
        'sub/subscription',
        json={'tier_id': new_tier_id, 'start_date': today}
    )
    assert new_subscription.status_code == 201

    subscription = client.get(
        f'/sub/subscription/{new_subscription.json()["id"]}'
    )
...
async def test_new_subscription_while_pending_payment(client):
    response = client.post('/sub/subscription', json={'tier_id': 1, 'start_date': today})
    data = response.json()
...
def test_update_subscription(client):
    ...
    response = client.patch(
        '/sub/subscription/1',
        json={'start_date': tomorrow, 'active': True}
    )
    data = response.json()
    print(data)
    assert response.status_code == 200
